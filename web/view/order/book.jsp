...
<c:set var="title" value="Book an Appointment" scope="request" />
...
<div style="max-width: 700px; margin: 3rem auto; padding: 0 1.5rem;">
    <div class="card">
        <div class="card-header">
            <h2 style="color: var(--primary); font-weight: 800; font-size: 1.75rem;">🚗 Book a New Wash Appointment</h2>
            <p style="color: var(--text-light); margin-top: 0.25rem; font-size: 0.95rem;">Select your preferred service and arrival time</p>
        </div>
        <div class="card-body">
            ...
            <form method="POST" action="${pageContext.request.contextPath}/order/book">
                <div class="form-group">
                    <label for="serviceId" class="form-label">Select Wash Service</label>
                    <select id="serviceId" name="serviceId" class="form-control" onchange="updateServicePreview()">
                        <option value="">-- Please select a service package --</option>
                ...
                <div class="form-group">
                    <label for="carPlate" class="form-label">License Plate (e.g. 29A-12345)</label>
                    <input type="text" id="carPlate" name="carPlate" class="form-control"
                           value="<c:out value="${carPlate}"/>" placeholder="Enter your vehicle license plate (at least 5 characters)">
                </div>

                <div class="form-group">
                    <label for="bookDate" class="form-label">Estimated Arrival Date & Time</label>
                ...
                <div style="margin-top: 2.5rem; display: flex; gap: 1rem; justify-content: flex-end;">
                    ...
                    <button type="submit" class="btn btn-primary" style="padding: 0.75rem 2rem; font-size: 1.05rem;">Confirm Appointment</button>
                </div>
            </form>
        </div>
    </div>
...
function updateServicePreview() {
    ...
    if (select.selectedIndex > 0) {
        ...
        const price = parseFloat(option.getAttribute("data-price")).toLocaleString("en-US");
