        ...
        <div class="filter-form-grid">
            ...
            <div class="search-input-wrapper">
                ...
                <input id="search" name="search" type="text" placeholder="Search"
                       class="search-input-field" value="<c:out value='${search}'/>">
            </div>
            ...
            <!-- Date -->
            <input id="dateFilter" name="date" type="date" class="filter-date-field"
                   value="<c:out value='${date}'/>">

            <!-- Sort By -->
            <select id="sortBy" name="sortBy" class="filter-select-field">
                <option value="created_desc" <c:if test="${empty sortBy or sortBy eq 'created_desc'}">
                        selected</c:if>>Newest Created</option>
                <option value="created_asc" <c:if test="${sortBy eq 'created_asc'}">selected</c:if>
                        >Oldest Created</option>
                    <option value="date_desc" <c:if test="${sortBy eq 'date_desc'}">selected</c:if>>Booking
                        Date (New -> Old)</option>
                    <option value="date_asc" <c:if test="${sortBy eq 'date_asc'}">selected</c:if>>Booking
                        Date (Old -> New)</option>
                    <option value="amount_desc" <c:if test="${sortBy eq 'amount_desc'}">selected</c:if>
                            >Total Amount (High -> Low)</option>
                    <option value="amount_asc" <c:if test="${sortBy eq 'amount_asc'}">selected</c:if>>Total
                        Amount (Low -> High)</option>
                </select>
            ...
            <table class="booking-table" id="bookingTable">
                ...
                <c:choose>
                    <c:when test="${not empty bookings}">
                        <c:forEach var="b" items="${bookings}">
                            <tr>
                                ...
                                <td>
                                    <div class="customer-cell">
                                        <div class="customer-avatar">
                                            <c:out
                                                value="${fn:substring(not empty b.user.fullname ? b.user.fullname : b.user.username, 0, 1).toUpperCase()}" />
                                        </div>
                                        <div>
                                            <div class="customer-name">
                                                <c:out
                                                    value="${not empty b.user.fullname ? b.user.fullname : b.user.username}" />
                                            </div>
                                            <c:set var="tierName"
                                                   value="${not empty b.user.loyaltyTier ? b.user.loyaltyTier.name : 'Member'}" />
                                            <span
                                                class="tier-badge-small tier-badge-${tierName.toUpperCase()}">
                                ...
                                <!-- Vehicle Brand + Model -->
                                <td class="td-vehicle">
                                    <c:out value="${b.vehicle.brand}" />
                                    <c:out value="${b.vehicle.model}" />
                                </td>
                                ...
                                <td class="td-schedule">
                                    ...
                                    <div
                                        class="schedule-time ${b.bookingStatus eq 'CANCELLED' ? 'cancelled-time' : ''}">
                                    ...
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            <button type="button"
                                                    ...
                                                    onclick="openCheckoutModal(this)" data-id="${b.id}"
                                                ...
                                                Collect
                                                <fmt:formatNumber value="${b.totalAmount}" type="number"
                                                                  groupingUsed="true" /> ₫
                                            </button>
                                        </c:otherwise>
                                    </c:choose>
                                ...
                                <td>
                                    <span class="status-badge status-${b.bookingStatus}">
                                        <c:choose>
                                            <c:when test="${b.bookingStatus eq 'CONFIRMED'}">Confirmed
                                            </c:when>
                                            <c:when test="${b.bookingStatus eq 'IN_PROGRESS'}">In
                                                Progress</c:when>
                                            <c:when test="${b.bookingStatus eq 'COMPLETED'}">Completed
                                            </c:when>
                                            <c:when test="${b.bookingStatus eq 'CANCELLED'}">Cancelled
                                            </c:when>
                                            <c:when test="${b.bookingStatus eq 'NO_SHOW'}">No Show
                                            </c:when>
                                ...
                                <td style="text-align: center;">
                                    <a href="<c:url value='/admin/bookings/detail?id=${b.id}'/>"
                                       class="btn-detail-icon" title="View details">
                                        <span class="material-symbols-outlined"
                                              style="font-size: 1.25rem;">arrow_forward</span>
                                    </a>
                                </td>
                            </tr>
                        </c:forEach>
                    ...
                    <c:otherwise>
                        <tr>
                            <td colspan="8"
                                style="text-align: center; color: #94A3B8; padding: 3rem; font-style: italic;">
                                No bookings found.
                            </td>
                        </tr>
                    </c:otherwise>
                                ...
                                <li>
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            ...
                                            <a href="${pageUrl}" class="page-item-btn"
                        ...
                        <c:otherwise>
                            <c:choose>
                                ...
                                <c:when test="${currentPage <= 3}">
                                    <c:forEach var="i" begin="1" end="4">
                                        <li>
                                            <c:choose>
                                                <c:when test="${i == currentPage}">
                                                    <button
                                                        class="page-item-btn active">${i}</button>
                                                </c:when>
                                                <c:otherwise>
                                                    <c:url value="${paginationBaseUrl}"
                                                           var="pageUrl">
                                                        <c:if test="${not empty search}">
                                                            <c:param name="search"
                                                                     value="${search}" />
                                    ...
                                    <li><span
                                            style="padding: 0.5rem; color: #64748B;">...</span>
                                    </li>
                                    <li>
                                        <c:url value="${paginationBaseUrl}" var="pageUrl">
                                            <c:if test="${not empty search}">
                                        ...
                                        <a href="${pageUrl}" class="page-item-btn"
                                           style="text-decoration: none; display: flex; align-items: center; justify-content: center;">${totalPages}</a>
                                    </li>
                                </c:when>
                                ...
                                <c:when test="${currentPage >= totalPages - 2}">
                                    <li>
                                        ...
                                        <a href="${pageUrl}" class="page-item-btn"
                                           style="text-decoration: none; display: flex; align-items: center; justify-content: center;">1</a>
                                    </li>
                                    <li><span
                                            style="padding: 0.5rem; color: #64748B;">...</span>
                                    </li>
                                    <c:forEach var="i" begin="${totalPages - 3}"
                                               end="${totalPages}">
                                        <li>
                                            <c:choose>
                                                <c:when test="${i == currentPage}">
...
<div id="paymentCheckoutModal" class="checkout-modal">
    <div class="checkout-modal-content">
        <div class="checkout-modal-header">
            ...
            <button type="button" class="checkout-modal-close"
                    onclick="closeCheckoutModal()">&times;</button>
        </div>

        <form id="checkoutPaymentForm" method="POST"
              action="<c:url value='/admin/bookings/collect-payment'/>">
            ...
            <div class="checkout-card">
                ...
                <div class="checkout-service-row">
                    <div class="checkout-service-left">
                        <div class="checkout-service-name" id="checkoutServiceName">Premium Detail Wash
                        </div>
                        <div class="checkout-service-duration">
                            <span class="material-symbols-outlined"
                                  style="font-size: 0.9rem; margin-right: 2px;">schedule</span>
                    ...
                    <div class="payment-options">
                        ...
                        <label class="payment-option-card" id="paymentCardBANK">
                            ...
                            <div class="payment-option-content">
                                <span class="custom-radio"></span>
                                <span
                                    class="material-symbols-outlined payment-icon">account_balance</span>
