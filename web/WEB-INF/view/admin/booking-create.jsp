...
<div class="admin-container">
    ...
    <div class="page-header-container">
        ...
        <div class="header-action-row">
            ...
            <div>
                <a href="<c:url value='/admin/bookings'/>" class="btn-admin-secondary" style="text-decoration: none;">
    ...
    <c:choose>
        ...
        <c:when test="${hasNoCustomer}">
            <div class="mgmt-card" style="padding: 1.5rem;">
                <h2 style="font-size: 1.2rem; font-weight: 700; color: #1E293B; margin-top: 0; margin-bottom: 1rem; display: flex; align-items: center; gap: 8px;">
                    <span class="material-symbols-outlined" style="color: #2563EB;">person_search</span>
                    Select a Customer to Proceed
                </h2>
                <p style="color: #64748B; font-size: 0.9rem; margin-bottom: 1.5rem;">Please find and choose the customer for whom you are creating this booking.</p>

                <!-- Search and Filter -->
                <div class="filter-card" style="margin-bottom: 1.5rem; background: #F8FAFC;">
                    <form action="<c:url value='/admin/bookings/new'/>" method="GET" style="margin: 0;">
                ...
                <!-- Customers Table -->
                <div style="overflow-x: auto; border: 1px solid var(--border-slate); border-radius: 8px;">
                                ...
                                <c:when test="${not empty customers}">
                                    <c:forEach var="u" items="${customers}">
                                        <tr>
                                            ...
                                            <!-- Total Washes -->
                                            <td class="td-washes" style="text-align: center;">${u.totalWashes}</td>

                                            <!-- Total Vehicles -->
                                            <td class="td-vehicle-count" style="text-align: center;">
                                                <span class="vehicle-count-tag">${u.vehicleCount}</span>
                                            </td>

                                            <!-- Action: Select -->
                                            <td style="text-align: center;">
                                                <a href="<c:url value='/admin/bookings/new'><c:param name='customerId' value='${u.id}'/></c:url>" class="admin-action-btn-sm">
                                                    Select
                                                </a>
                                            </td>
                                        </tr>
                ...
                <c:if test="${totalPages > 0}">
                    <div class="pagination-container" style="border-top: 1px solid var(--border-slate); margin-top: 1rem; padding-top: 1rem;">
                                ...
                                <c:choose>
                                    <c:when test="${totalPages <= 5}">
                                        <c:forEach var="i" begin="1" end="${totalPages}">
                                            <li>
                                                <c:url value="/admin/bookings/new" var="pageUrl">
                                                    <c:if test="${not empty search}"><c:param name="search" value="${search}"/></c:if>
                                                    <c:if test="${not empty selectedTierId}"><c:param name="tierId" value="${selectedTierId}"/></c:if>
                                                    <c:param name="page" value="${i}"/>
                                                </c:url>
                                                <a href="${pageUrl}" class="page-item-btn ${currentPage == i ? 'active' : ''}" style="text-decoration: none;">${i}</a>
                                            </li>
                                        </c:forEach>
                                    </c:when>
                                    <c:otherwise>
            ...
            <div class="admin-booking-card">
                ...
                <div class="customer-info-box">
                    <div class="customer-details">
                        <div class="customer-avatar">
                            <c:out value="${fn:substring(customer.username, 0, 2).toUpperCase()}"/>
                        </div>
                        <div class="customer-meta">
                            <h3><c:out value="${customer.fullname}"/></h3>
                            <p>@<c:out value="${customer.username}"/> • <c:out value="${not empty customer.phone ? customer.phone : 'No Phone'}"/> • <span class="tier-badge tier-${not empty customer.loyaltyTier ? customer.loyaltyTier.name : 'Member'}" style="transform: scale(0.85); display: inline-block;"><c:out value="${not empty customer.loyaltyTier ? customer.loyaltyTier.name : 'Member'}"/></span></p>
                        </div>
                    </div>
                    <div>
                        <a href="<c:url value='/admin/bookings/new'/>" class="btn-admin-secondary" style="font-size: 0.85rem; padding: 0.4rem 0.8rem; text-decoration: none;">
                ...
                <c:if test="${not empty bookingError}">
                    <div class="alert alert-error" style="background-color: #FEF2F2; color: #DC2626; padding: 1rem; border-radius: 8px; margin-bottom: 1.5rem; border: 1px solid #FEE2E2;">
                ...
                <main class="booking-page">
                    <form id="bookingForm" method="POST" action="<c:url value='/admin/bookings/create'/>">
                        <!-- Hidden customerId -->
                            ...
                            <div class="booking-selections">
                                <section class="selection-card">
                                    <div class="section-title">
                                        ...
                                        <span style="color: #64748B; font-size: 0.85rem;">Customer vehicles list</span>
                                    </div>

                                    <c:choose>
                                        <c:when test="${empty vehicles}">
                                            <div class="empty-state compact" style="text-align: center; padding: 2rem;">
                                                <h3 style="font-size: 1rem; color: #64748B;">No vehicle registered for this customer</h3>
                                                <p style="font-size: 0.85rem; color: #94A3B8;">Customer must add a vehicle first.</p>
                                            </div>
                                        </c:when>
                                        <c:otherwise>
                                ...
                                <section class="selection-card">
                                    ...
                                    <c:choose>
                                        <c:when test="${empty services}">
                                            <div class="empty-state compact" style="text-align: center; padding: 2rem;">
                                                <h3 style="font-size: 1rem; color: #64748B;">No service available</h3>
                                            </div>
                                        </c:when>
                                        <c:otherwise>
                                ...
                                <div class="date-time-grid">
                                    <section class="selection-card">
                                        ...
                                        <div class="membership-note">
                                            <c:out value="${empty customer.loyaltyTier ? 'Member' : customer.loyaltyTier.name}"/> Member:
                                    ...
                                    <section class="selection-card">
                                        ...
                                        <div id="timeMessage" class="helper-text">
                                            <c:choose>
                                                <c:when test="${empty selectedService || empty selectedDate}">
