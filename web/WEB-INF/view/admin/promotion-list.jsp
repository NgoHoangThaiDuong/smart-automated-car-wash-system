...
<div class="admin-container">
    ...
    <div class="page-header-container">
        ...
        <div class="header-action-row">
            ...
            <div style="display: flex; gap: 0.75rem;">
                ...
                <a href="<c:url value='/admin/promotions/new'/>" class="btn-admin-primary" style="text-decoration: none;">
    ...
    <c:if test="${not empty promotionMessage}">
        <div class="alert-box alert-success" style="margin-bottom: 1.5rem;">
            <span><c:out value="${promotionMessage}"/></span>
                    ...
                    <c:choose>
                        <c:when test="${not empty promotions}">
                            <c:forEach var="promotion" items="${promotions}">
                                <tr>
                                    <!-- ID -->
                                    <td style="font-weight: 600; color: var(--text-slate-muted);">${promotion.id}</td>
                                    ...
                                    <td>
                                        <c:choose>
                                            <c:when test="${promotion.discountType == 'PERCENT'}">
                                                <span class="discount-type-tag discount-type-percent">
                                                    <span class="material-symbols-outlined" style="font-size: 0.9rem;">percent</span>
                                                    Percent
                                                </span>
                                            </c:when>
                                            <c:otherwise>
                                                <span class="discount-type-tag discount-type-fixed">
                                                    <span class="material-symbols-outlined" style="font-size: 0.9rem;">payments</span>
                                                    Fixed
                                                </span>
                                            </c:otherwise>
                                        </c:choose>
                                    ...
                                    <td style="text-align: right;">
                                        <span class="discount-value">
                                            <c:choose>
                                                <c:when test="${promotion.discountType == 'PERCENT'}">
                                                    <fmt:formatNumber value="${promotion.discountValue}" maxFractionDigits="2"/>%
                                                </c:when>
                                                <c:otherwise>
                                    ...
                                    <td>
                                        <c:choose>
                                            <c:when test="${empty promotion.targetTierId}">
                                                <span class="promo-tier-badge">All Tiers</span>
                                            </c:when>
                                            <c:otherwise>
                                    ...
                                    <td style="text-align: center;">
                                        <c:choose>
                                            <c:when test="${promotion.status == 'ACTIVE'}">
                                                <span class="promo-badge promo-badge-active">Active</span>
                                            </c:when>
                                            <c:otherwise>
                                                <span class="promo-badge promo-badge-inactive">Inactive</span>
                                            </c:otherwise>
                                        </c:choose>
                                    </td>
