...
<body>
    ...
    <div class="main-container">
        ...
        <div class="card">
            ...
            <div id="vehicle-alert" class="alert">
                ...
                <form id="addVehicleForm" method="POST" action="${pageContext.request.contextPath}/vehicles/add">
                    <div class="form-grid">
                        <div class="form-field">
                            ...
                            <input type="text" id="add-plate" name="licensePlate" placeholder="29A-12345" required pattern="[0-9]{2}[A-Za-z]{1,2}-[0-9]{4,6}" title="Định dạng: xxyy-xxxx (VD: 29A-12345)">
                        </div>
                        <div class="form-field">
                            <label>Hãng xe *</label>
                            <input type="text" id="add-brand" name="brand" placeholder="Toyota" required>
                        </div>
                        <div class="form-field">
                            <label>Dòng xe *</label>
                            <input type="text" id="add-model" name="model" placeholder="Camry" required>
                        </div>
                        <div class="form-field">
                            <label>Màu xe *</label>
                            <input type="text" id="add-color" name="color" placeholder="Trắng" required>
                        </div>
                    </div>
                    <div class="form-actions">
                        <button type="button" class="btn-form btn-form-cancel" onclick="closeForms()">Hủy</button>
                        <button type="submit" id="addSubmitBtn" class="btn-form btn-form-submit" disabled style="opacity: 0.5; cursor: not-allowed;">
                            ...
                            <div class="vehicle-item">
                                <div class="vehicle-meta">
                                    ...
                                    <div class="vehicle-text">
                                        ...
                                        <span class="vehicle-type"><c:out value="${v.vehicleType}"/> • <c:out value="${v.color}"/></span>
                                    </div>
                                </div>
                                <div class="vehicle-actions">
                                    <button class="btn-icon btn-edit" title="Edit vehicle info" onclick="showEditForm(${v.id}, '<c:out value="${v.licensePlate}"/>', '<c:out value="${v.vehicleType}"/>', '<c:out value="${v.color}"/>')">
    ...
    <script>
        ...
        function validateAddForm() {
            const plate = document.getElementById('add-plate');
            const brand = document.getElementById('add-brand');
            const model = document.getElementById('add-model');
            const color = document.getElementById('add-color');
            const submitBtn = document.getElementById('addSubmitBtn');

            if(plate) plate.value = plate.value.toUpperCase();
            if(brand) capitalizeFirstLetter(brand);
            if(model) capitalizeFirstLetter(model);
            if(color) capitalizeFirstLetter(color);

            const plateRegex = /^[0-9]{2}[A-Za-z]{1,2}-[0-9]{4,6}$/;
            const isValid = plateRegex.test(plate.value) && 
                            brand.value.trim() !== '' && 
                            model.value.trim() !== '' && 
                            color.value.trim() !== '';
        ...
        document.getElementById('add-plate').addEventListener('input', validateAddForm);
        document.getElementById('add-brand').addEventListener('input', validateAddForm);
        document.getElementById('add-model').addEventListener('input', validateAddForm);
        document.getElementById('add-color').addEventListener('input', validateAddForm);

        // Show spinner on Add Submit
        ...
        // Show spinner on Profile Submit
